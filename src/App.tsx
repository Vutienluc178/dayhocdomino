                ...
                <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-200 flex flex-wrap items-center justify-between gap-4">
                  ...
                  {previewMode.includes('worksheet') && (
                    <>
                      <div className="flex items-center gap-3 bg-slate-100 p-1 rounded-2xl">
                        <button 
                          onClick={() => setWorksheetType('exercise')}
                          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${worksheetType === 'exercise' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                          Bài tập
                        </button>
                        <button 
                          onClick={() => setWorksheetType('lesson')}
                          className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${worksheetType === 'lesson' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                          Bài học
                        </button>
                      </div>
                      <div className="flex items-center gap-3 bg-slate-100 p-1 rounded-2xl">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 px-2">Dòng kẻ:</span>
                        <button 
                          onClick={() => setWorksheetLines(prev => Math.max(1, prev - 1))}
                          className="p-2 bg-white text-indigo-600 rounded-xl shadow-sm hover:bg-indigo-50 transition-all"
                          title="Giảm dòng kẻ"
                        >
                          <Minus size={14} />
                        </button>
                        <span className="text-sm font-black text-indigo-600 w-8 text-center">{worksheetLines}</span>
                        <button 
                          onClick={() => setWorksheetLines(prev => Math.min(5, prev + 1))}
                          className="p-2 bg-white text-indigo-600 rounded-xl shadow-sm hover:bg-indigo-50 transition-all"
                          title="Tăng dòng kẻ"
                        >
                          <Plus size={14} />
...
const WorksheetRenderer = ({ data, lines = 2, type = 'exercise' }: { data: GameData[], lines?: number, type?: 'exercise' | 'lesson' }) => {
  ...
  return (
    <div className="bg-slate-100 space-y-8 print:space-y-0 print:bg-white flex flex-col items-center">
      {pages.map((pageQuestions, pageIndex) => (
        <div key={pageIndex} className="game-page bg-white p-8 w-[210mm] min-h-[297mm] text-black font-serif shadow-lg print:shadow-none print:p-8 relative box-border flex flex-col print:break-after-page">
              ...
              <div className="flex flex-col gap-3 text-[11pt] mb-3 px-2">
                ...
                {type === 'lesson' ? (
                  <div className="flex flex-col w-full mt-2">
                    <span className="font-bold text-[11pt] mb-1 italic">Phần ghi chép kiến thức trọng tâm:</span>
                    <div className="border-b border-dotted border-black w-full h-6"></div>
                    <div className="border-b border-dotted border-black w-full h-6"></div>
                    <div className="border-b border-dotted border-black w-full h-6"></div>
                    <div className="border-b border-dotted border-black w-full h-6"></div>
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-6 mt-4 flex-1 content-start">
          ...
          {pageIndex === pages.length - 1 && (
            <div className="mt-8 pt-4 border-t-2 border-black shrink-0">
